...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("用法: go run gen_bson_v2.go <filename.go>")
	}
	fileName := os.Args[1]
	...
	if err != nil {
		log.Fatalf("解析文件错误: %v", err)
	}

	// 1. Populate structMap and initial interface implementations based on //go:bson tags
	...
	// 2. Scan for existing MarshalBSON / UnmarshalBSON method declarations in the AST
	for _, decl := range node.Decls {
	...
	// First pass: collect all struct names into the whitelist.
	for _, decl := range node.Decls {
		genDecl, ok := decl.(*ast.GenDecl)
		if !ok || genDecl.Tok != token.TYPE {
			continue
	...
	// Second pass: collect field info (whitelist is fully populated).
	for _, decl := range node.Decls {
		genDecl, ok := decl.(*ast.GenDecl)
		if !ok || genDecl.Tok != token.TYPE {
			continue
		...
		for _, spec := range genDecl.Specs {
			...
			collectFields(structType, fset, structWhiteList, &sInfo)
	...
	fmt.Fprintf(&buf,
		"// Code generated by gen_bson_v2.go. DO NOT EDIT.\npackage %s\n\n", node.Name.Name)
...
func hasBsonDirective(gd *ast.GenDecl) bool {
	...
	for _, c := range gd.Doc.List {
		if strings.TrimSpace(c.Text) == "//go:bson" {
			return true
		}
	}
	return false
...
func generateUnmarshal(buf *bytes.Buffer, s *structInfo) {
	...
	buf.WriteString("\tvar (\n\t\telems []bsoncore.Element\n\t\terr   error\n\t)\n")
	buf.WriteString("\telems, err = bsoncore.Document(b).Elements()\n\tif err != nil {\n\t\treturn err\n\t}\n")
	buf.WriteString("\tfor _, elem := range elems {\n\t\tswitch elem.Key() {\n")

	for _, f := range s.Fields {
		if f.BsonKey == "" {
			continue
		}
		genUnmarshalCase(buf, &f, "\t\t")
	}

	buf.WriteString("\t\tdefault:\n")
	buf.WriteString("\t\t\t// unknown field — skip for forward compatibility\n")
	buf.WriteString("\t\t}\n\t}\n\treturn nil\n}\n\n")
}

func genUnmarshalCase(buf *bytes.Buffer, f *fieldInfo, ind string) {
	ind2 := ind + "\t"
	ind3 := ind2 + "\t"
	...
	switch f.Category {
	case catDouble:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().Double()\n", ind2, f.Name)
	case catString:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().StringValue()\n", ind2, f.Name)
	case catBoolean:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().Boolean()\n", ind2, f.Name)
	case catInt32:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().Int32()\n", ind2, f.Name)
	case catInt64:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().AsInt64()\n", ind2, f.Name)
	case catInt:
		fmt.Fprintf(buf, "%sz.%s = int(elem.Value().AsInt64())\n", ind2, f.Name)
	case catInt8:
		fmt.Fprintf(buf, "%sz.%s = int8(elem.Value().AsInt64())\n", ind2, f.Name)
	case catInt16:
		fmt.Fprintf(buf, "%sz.%s = int16(elem.Value().AsInt64())\n", ind2, f.Name)
	case catUint:
		fmt.Fprintf(buf, "%sz.%s = uint(elem.Value().AsInt64())\n", ind2, f.Name)
	case catUint16:
		fmt.Fprintf(buf, "%sz.%s = uint16(elem.Value().AsInt64())\n", ind2, f.Name)
	case catUint32:
		fmt.Fprintf(buf, "%sz.%s = uint32(elem.Value().AsInt64())\n", ind2, f.Name)
	case catUint64:
		fmt.Fprintf(buf, "%sz.%s = uint64(elem.Value().AsInt64())\n", ind2, f.Name)
	case catByte:
		fmt.Fprintf(buf, "%sz.%s = byte(elem.Value().AsInt64())\n", ind2, f.Name)
	case catFloat32:
		fmt.Fprintf(buf, "%sz.%s = float32(elem.Value().Double())\n", ind2, f.Name)
	case catDateTime:
		fmt.Fprintf(buf, "%sz.%s = primitive.DateTime(elem.Value().DateTime()).Time()\n", ind2, f.Name)
	case catPrimitiveDateTime:
		fmt.Fprintf(buf, "%sz.%s = primitive.DateTime(elem.Value().DateTime())\n", ind2, f.Name)
	case catObjectID:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().ObjectID()\n", ind2, f.Name)
	case catBinary:
		if f.BinaryIsNative {
			fmt.Fprintf(buf, "%sif elem.Value().Type == 0x0A {\n", ind2)
			fmt.Fprintf(buf, "%sz.%s = nil\n", ind3, f.Name)
			fmt.Fprintf(buf, "%s} else {\n", ind2)
			fmt.Fprintf(buf, "%s_, z.%s, _ = elem.Value().BinaryOK()\n", ind3, f.Name)
			fmt.Fprintf(buf, "%s}\n", ind2)
		} else {
			fmt.Fprintf(buf, "%ssubtype, data, _ := elem.Value().BinaryOK(); z.%s = primitive.Binary{Subtype: subtype, Data: data}\n", ind2, f.Name)
		}
	case catRegex:
		fmt.Fprintf(buf, "%spattern, options := elem.Value().Regex(); z.%s = primitive.Regex{Pattern: pattern, Options: options}\n", ind2, f.Name)
	case catTimestamp:
		fmt.Fprintf(buf, "%st, i := elem.Value().Timestamp(); z.%s = primitive.Timestamp{T: t, I: i}\n", ind2, f.Name)
	case catDecimal128:
		fmt.Fprintf(buf, "%sz.%s = elem.Value().Decimal128()\n", ind2, f.Name)
	case catJavaScript:
		fmt.Fprintf(buf, "%sz.%s = primitive.JavaScript(elem.Value().JavaScript())\n", ind2, f.Name)
	case catSymbol:
		fmt.Fprintf(buf, "%sz.%s = primitive.Symbol(elem.Value().Symbol())\n", ind2, f.Name)
	case catNull:
	...
	case catJavaScriptWithScope:
		fmt.Fprintf(buf, "%scode, scope := elem.Value().CodeWithScope(); z.%s = primitive.CodeWithScope{Code: primitive.JavaScript(code), Scope: scope}\n", ind2, f.Name)

	case catPointer:
		genUnmarshalPtr(buf, f, ind2, ind3)
	...
	case catArray:
		...
		fmt.Fprintf(buf, "%s}\n", ind3)
		fmt.Fprintf(buf, "%sarrBytes, ok := elem.Value().ArrayOK()\n", ind3)
	...
	case catMap:
		...
		fmt.Fprintf(buf, "%sbreak\n", ind3+"\t")
		fmt.Fprintf(buf, "%s}\n", ind3)
		fmt.Fprintf(buf, "%smapBytes, ok := elem.Value().DocumentOK()\n", ind3)
	...
	case catPrimitiveD, catPrimitiveM:
		fmt.Fprintf(buf, "%s{\n", ind2)
		fmt.Fprintf(buf, "%ssubBytes, ok := elem.Value().DocumentOK()\n", ind3)
		fmt.Fprintf(buf, "%sif !ok { return fmt.Errorf(%q, %q) }\n", ind3, "字段 %s 不是文档类型", f.BsonKey)
	...
	case catStructRef:
		...
		if unmarshalerTypes[f.StructName] {
			...
			genUnmarshalStructValue(buf, f.Fields, "elem.Value()", "z."+f.Name, ind3, f.BsonKey)
		}
		fmt.Fprintf(buf, "%s}\n", ind2)
	case catAnonStruct:
		fmt.Fprintf(buf, "%s{\n", ind2)
...
func genUnmarshalPtr(buf *bytes.Buffer, f *fieldInfo, ind2, ind3 string) {
	elem := f.ElemCat
	fmt.Fprintf(buf, "%s{\n", ind2)
	fmt.Fprintf(buf, "%sif elem.Value().Type == 0x0A {\n", ind3)
	fmt.Fprintf(buf, "%sz.%s = nil\n", ind3+"\t", f.Name)
	...
	fmt.Fprintf(buf, "%s}\n", ind3)
	if elem.Category == catStructRef {
		fmt.Fprintf(buf, "%sval, ok := elem.Value().DocumentOK()\n", ind3)
	...
	case catString:
		fmt.Fprintf(buf, "%stmp := elem.Value().StringValue(); z.%s = &tmp\n", ind3, f.Name)
	case catInt32:
		fmt.Fprintf(buf, "%stmp := elem.Value().Int32(); z.%s = &tmp\n", ind3, f.Name)
	case catInt64:
		fmt.Fprintf(buf, "%stmp := elem.Value().AsInt64(); z.%s = &tmp\n", ind3, f.Name)
	case catInt:
		fmt.Fprintf(buf, "%stmp := int(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catInt8:
		fmt.Fprintf(buf, "%stmp := int8(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catInt16:
		fmt.Fprintf(buf, "%stmp := int16(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catUint:
		fmt.Fprintf(buf, "%stmp := uint(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catUint16:
		fmt.Fprintf(buf, "%stmp := uint16(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catUint32:
		fmt.Fprintf(buf, "%stmp := uint32(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catUint64:
		fmt.Fprintf(buf, "%stmp := uint64(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catByte:
		fmt.Fprintf(buf, "%stmp := byte(elem.Value().AsInt64()); z.%s = &tmp\n", ind3, f.Name)
	case catBoolean:
		fmt.Fprintf(buf, "%stmp := elem.Value().Boolean(); z.%s = &tmp\n", ind3, f.Name)
	case catDouble:
		fmt.Fprintf(buf, "%stmp := elem.Value().Double(); z.%s = &tmp\n", ind3, f.Name)
	case catFloat32:
		fmt.Fprintf(buf, "%stmp := float32(elem.Value().Double()); z.%s = &tmp\n", ind3, f.Name)
	case catDateTime:
		fmt.Fprintf(buf, "%stmp := primitive.DateTime(elem.Value().DateTime()).Time(); z.%s = &tmp\n", ind3, f.Name)
	case catPrimitiveDateTime:
		fmt.Fprintf(buf, "%stmp := primitive.DateTime(elem.Value().DateTime()); z.%s = &tmp\n", ind3, f.Name)
	case catObjectID:
		fmt.Fprintf(buf, "%stmp := elem.Value().ObjectID(); z.%s = &tmp\n", ind3, f.Name)
	default:
		fmt.Fprintf(buf, "%sz.%s = nil\n", ind3, f.Name)
	}
	fmt.Fprintf(buf, "%s}\n", ind2)
